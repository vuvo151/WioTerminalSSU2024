...
#define AIO_KEY         "aio_xxxxxxxxxxxxxxxxxxxxxxxxxxx"
WiFiClient client;
...
Adafruit_MQTT_Subscribe ledstatus = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/ledstatus");
...
void loop() {
  ...
  while ((subscription = mqtt.readSubscription(5000))) {
    ...
    // check if its the ledbrightness feed
    if (subscription == &ledbrightness) {
      Serial.print(F("ledbrightness: "));
      Serial.println((char *)ledbrightness.lastread);
      uint16_t ledbrightnessval = atoi((char *)ledbrightness.lastread);  // convert to a number
      analogWrite(A0, ledbrightnessval);
    }
  }
